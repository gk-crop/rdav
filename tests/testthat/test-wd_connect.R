...
test_that("connection works", {
  resp <- httr2::with_mocked_responses(webdav_mock_response_connect,
                                    wd_connect("https://cloud.example.com", "abc", "123") |>
                                      httr2::req_method("HEAD") |> httr2::req_perform())
  expect_equal(httr2::resp_status(resp),200)
})

test_that("url is changed", {
  expect_error(
    httr2::with_mocked_responses(webdav_mock_response_connect,
...
test_that("credential are changed", {
  ...
  expect_error(resp <- httr2::with_mocked_responses(webdav_mock_response_connect,
